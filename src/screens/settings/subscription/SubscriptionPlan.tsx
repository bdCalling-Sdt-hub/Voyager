...
const SubscriptionPlan = ({navigation}: any) => {
  ...
  const [activeTab, setActiveTab] = useState<
    'my_subscription' | 'invoice_history'
  >('my_subscription');
  ...
  const plans =
    subscriptionData?.data?.map((item: any) => ({
      id: item.id,
  ...
  if (
    ...
    (activeTab === 'invoice_history' && isInvoicesLoading)
  ) {
    return (
      <View
        style={tw`flex-1 justify-center items-center bg-white dark:bg-primaryDark`}>
  ...
  if (
    profileError ||
    subscriptionError ||
    (activeTab === 'invoice_history' && invoicesError)
  ) {
    return (
      <View
        style={tw`flex-1 justify-center items-center bg-white dark:bg-primaryDark`}>
  ...
  const renderPlanItem = ({item}: {item: any}) => (
    <View style={[tw`justify-center `, {width: screenWidth}]}>
      <View
        style={[
          tw`bg-gray80 dark:bg-darkBg p-4 rounded-2xl ml-3 `,
        ...
        <View style={tw`flex-row items-center justify-between`}>
          ...
          <View>
            ...
            <Text
              style={tw`text-gray100 dark:text-white text-xs font-WorkRegular`}>
              {item.status === 'Active'
        ...
        <View style={tw`mt-6 gap-y-4`}>
          {item.features?.map((feature: string, index: number) => (
            <View key={index} style={tw`flex-row items-center gap-3`}>
              ...
              <Text
                style={tw`text-gray70 dark:text-white text-base font-WorkMedium`}>
                {feature}
              </Text>
            </View>
          ))}
        ...
        {item.status === 'Active' && (
          <Text
            style={tw`text-gray100 dark:text-white text-sm font-WorkRegular mt-4`}>
          ...
          {item.status === 'Active' ? (
            <>
              <TouchableOpacity
                ...
                <Text
                  style={tw`text-center text-violet100 text-base font-WorkMedium`}>
                  Change Plan
                </Text>
              </TouchableOpacity>
              <TouchableOpacity
            ...
            <TouchableOpacity
              style={tw`bg-violet100 border border-violet100 rounded-full p-3 mt-2`}
              onPress={() =>
                navigation.navigate('UpgradePlan', {planId: item.id})
              }>
              <Text
                style={tw`text-center text-white text-base font-WorkMedium`}>
          ...
          <View
            ...
            {plans.map((_, index) => (
              <View
                ...
                style={[
                  index === currentIndex
                    ? tw`bg-violet100 rounded-full w-4 h-2 mx-1`
                    : tw`mx-1 w-2 h-2 bg-gray-600 rounded-full`,
                ]}
              />
            ))}
          </View>
  ...
  return (
    <View style={tw`bg-white h-full px-[4%] pb-2 dark:bg-primaryDark`}>
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View
          style={tw`flex-row bg-gray80 dark:bg-darkBg p-1 rounded-full mb-4`}>
          <TouchableOpacity
            style={tw`${
              activeTab === 'my_subscription' ? 'bg-violet100' : ''
            } py-4 rounded-full flex-1 justify-center items-center`}
            onPress={() => setActiveTab('my_subscription')}>
            <Text
              style={tw`${
                activeTab === 'my_subscription' ? 'text-white' : 'text-gray100'
          ...
          <TouchableOpacity
            style={tw`${
              activeTab === 'invoice_history' ? 'bg-violet100' : ''
            } py-4 rounded-full flex-1 justify-center items-center`}
            onPress={() => setActiveTab('invoice_history')}>
            <Text
              style={tw`${
                activeTab === 'invoice_history' ? 'text-white' : 'text-gray100'
              } text-xs font-WorkMedium`}>
              Invoice History
            </Text>
          </TouchableOpacity>
